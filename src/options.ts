...
class Options extends EventEmitter {
  constructor() {
    ...
    if (!hasFile) fs.copyFileSync(this._dirname + '/src/options.default.json', this._dirname + '/options/options.json')
    // 读取默认设置文件
    const defaultOptionBuffer = fs.readFileSync(this._dirname + '/src/options.default.json')
